...
  body('password', 'Password must be at least 5 characters').isLength({ min: 5 }), // Validate password
], async (req, res) => {
...
    const user = await User.create({ name: req.body.name, password: secPass, email: req.body.email }); // Create user
...
    if (!passCompare) return res.status(400).json({ error: "Invalid credentials" }); // Incorrect password
